...
const REVIEWS = [
  { name: "Marcus T.", rating: 5, text: "Best haircut I've had in years. The attention to detail is unmatched. Truly a premium experience." },
  { name: "Daniel R.", rating: 5, text: "The hot towel shave is a ritual. I leave feeling like a new man every single time." },
  { name: "Anthony L.", rating: 5, text: "Atmosphere, skill, and service are all top-tier. Legend lives up to the name." },
];
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0">
        <img src={heroImg} alt="" width={1920} height={1280} className="h-full w-full object-cover blur-sm scale-110" />
      ...
      <div className="relative z-10 text-center px-6 max-w-4xl animate-in fade-in slide-in-from-bottom-4 duration-1000">
        <p className="text-gold tracking-[0.4em] text-xs md:text-sm mb-6 font-medium">— EST. 2010 —</p>
    ...
    <section id="services" className="py-24 px-6">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold mb-4">Premium <span className="text-gradient-gold">Services</span></h2>
          ...
          {services.map((s) => (
            <div key={s.id} className="group relative bg-card border border-border rounded-2xl p-7 hover:border-gold/50 transition-all duration-500 hover:-translate-y-1 shadow-elegant">
              <div className="flex items-start justify-between mb-4">
                ...
                <span className="text-2xl font-display text-gradient-gold font-bold">${s.price}</span>
              </div>
              <p className="text-sm text-muted-foreground mb-4 leading-relaxed">{s.description}</p>
              <div className="flex items-center justify-between pt-4 border-t border-border/50">
                <span className="text-xs text-muted-foreground">{s.duration}</span>
                <Link to="/book" search={{ service: s.id } as never} className="text-xs text-gold hover:underline">Book →</Link>
              </div>
            </div>
          ))}
        </div>
    ...
    <section id="reviews" className="py-24 px-6 bg-card/30">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold">What Our <span className="text-gradient-gold">Clients</span> Say</h2>
        </div>
    ...
    <section id="contact" className="py-24 px-6">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold">Visit <span className="text-gradient-gold">Us</span></h2>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <div className="space-y-6">
            <div className="bg-card border border-border rounded-2xl p-6 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">123 King Street, Downtown<br />New York, NY 10001</p>
              </div>
            </div>
            <div className="bg-card border border-border rounded-2xl p-6 flex items-start gap-4">
          ...
          <form onSubmit={onSubmit} className="bg-card border border-border rounded-2xl p-8 space-y-4 shadow-elegant">
